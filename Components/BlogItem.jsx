...
function BlogItem({ title, description, category, image, _id }) {
  return (
    <div className="max-w-[330px] sm:max-w-[300px] bg-white border border-black hover:shadow-[-7px_7px_0px_#000] transition-all duration-300">
      ...
      <div className="p-5">
        <h4 className="mb-2 text=lg font-medium tracking-tight text-gray-900 h-12 card-title">
          {title}
        </h4>
        <p className="mb-3 text-sm text-gray-700 tracking-tight card-description h-[60px]">
          {description}
        </p>
        <Link
          href={`/blogs/${_id}`}
